...
class RlController:
    ...
    def __init__(self, cfg: Union[DictConfig, ListConfig]):
        ...
        self.default_joint_positions = np.array([self.cfg.default_joint_positions[i] for i in self.cfg.action_indices],dtype=np.float32)
        '''


        if self.cfg.num_actions == self.cfg.num_joints:
            # All joints controlled, use full default_joint_positions
            self.default_joint_positions = np.array(
                self.cfg.default_joint_positions, dtype=np.float32
            )
        elif self.cfg.num_actions == 12 and self.cfg.num_joints == 14:
            # Policy controls only subset of joints (skipping toes)
            self.default_joint_positions = np.array(
                [self.cfg.default_joint_positions[i] for i in self.cfg.action_indices],
                dtype=np.float32
            )
        else:
            self.default_joint_positions = np.array(self.cfg.default_joint_positions[10:], dtype=np.float32)
